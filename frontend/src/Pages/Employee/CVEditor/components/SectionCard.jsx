...
import { GripVertical, ChevronDown, X, Plus, Trash2 } from 'lucide-react';
import { getSectionMeta, inpCls, txtCls, newExp, newEdu, newCustomItem } from '../constants';
import TagInput from './TagInput';
import Field from './Field';
import RepeatableItem from './RepeatableItem';
import AddBtn from './AddBtn';

export default function SectionCard({ sectionKey, form, handlers, onRemove, collapsed, onToggleCollapse }) {
              ...
              <RepeatableItem key={i} onDelete={() => exp.remove(i)} onMoveUp={() => exp.moveUp(i)}
                ...
                <div className="grid grid-cols-2 gap-2">
                  <Field label="From Year">
                    <input className={`${inpCls}`} value={item.durationFrom} onChange={(e) => exp.update(i, 'durationFrom', e.target.value)} placeholder="2021" />
                  </Field>
                  <Field label="To Year">
                    <input className={`${inpCls}`} value={item.durationTo} onChange={(e) => exp.update(i, 'durationTo', e.target.value)} placeholder="Present" />
                  </Field>
                </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            {form.education.map((item, i) => (
              <RepeatableItem key={i} onDelete={() => edu.remove(i)} onMoveUp={() => edu.moveUp(i)}
                ...
                <div className="grid grid-cols-2 gap-2">
                  <Field label="From Year">
                    <input className={`${inpCls}`} value={item.durationFrom} onChange={(e) => edu.update(i, 'durationFrom', e.target.value)} placeholder="2018" />
                  </Field>
                  <Field label="To Year">
                    <input className={`${inpCls}`} value={item.durationTo} onChange={(e) => edu.update(i, 'durationTo', e.target.value)} placeholder="2022" />
                  </Field>
                </div>
                ...
                <div className="p-3 flex flex-col gap-2">
                  {section.items.map((item, ii) => (
                    <div key={ii} className="border border-[var(--border-color)] p-2.5 bg-[var(--card-bg)] flex flex-col gap-2">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <Field label="From Year">
                          <input className={inpCls} value={item.durationFrom} onChange={(e) => updateCustomItem(si, ii, 'durationFrom', e.target.value)} placeholder="2022" />
                        </Field>
                        <Field label="To Year">
                          <input className={inpCls} value={item.durationTo} onChange={(e) => updateCustomItem(si, ii, 'durationTo', e.target.value)} placeholder="2024" />
                        </Field>
                      </div>
                      <Field label="Description">
