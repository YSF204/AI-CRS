      ...
      <div className="flex items-center border-b-4 border-[var(--nm-ink)] flex-shrink-0 overflow-hidden"
        ...
        {sidebarOpen && (
          <div className="px-5 flex-1 overflow-hidden">
            <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-[var(--nm-text-tertiary)] whitespace-nowrap font-bold">Registry</p>
            <p className="font-[var(--font-display)] font-black text-xs mt-0.5 whitespace-nowrap uppercase">Add Block →</p>
          </div>
        )}
        ...
        <button
          ...
          style={{ borderLeft: sidebarOpen ? '4px solid var(--nm-ink)' : 'none' }}
        >
          <div style={{
            transition: 'transform 0.28s cubic-bezier(0.4,0,0.2,1)',
            transform: sidebarOpen ? 'none' : 'rotate(180deg)',
          }}>
            <PanelLeftClose size={18} className="text-[var(--nm-text-tertiary)]" strokeWidth={2.5} />
          </div>
        </button>
      </div>
          ...
          return (
            <button
              ...
              style={{
                ...
                borderLeft: active && sidebarOpen ? '6px solid var(--nm-ink)' : '6px solid transparent',
                marginBottom: '2px'
              }}
              onMouseEnter={e => { if (!active) e.currentTarget.style.background = 'var(--nm-surface-high)'; }}
            ...
            >
              ...
              <Icon size={16} style={{ color: active ? textColor : 'var(--nm-text-tertiary)', flexShrink: 0 }} strokeWidth={2.5} />
              {sidebarOpen && (
                <>
                  <span className="font-mono text-[11px] font-bold uppercase tracking-wider flex-1 whitespace-nowrap overflow-hidden"
                    style={{ color: active ? textColor : 'var(--nm-text-primary)' }}>
                    {label}
                  </span>
                  {active && (
      ...
      {sidebarOpen && (
        <div className="px-5 py-4 border-t-4 border-[var(--nm-ink)] flex-shrink-0 bg-[var(--nm-surface-low)]">
          <p className="font-mono text-[10px] text-[var(--nm-text-tertiary)] whitespace-nowrap uppercase font-bold">
            <span className="font-black text-[var(--nm-text-primary)]">{activeSections.length}</span> / {ALL_SECTIONS.length} ACTIVE BLOCKS
          </p>
        </div>
      )}
    </div>
