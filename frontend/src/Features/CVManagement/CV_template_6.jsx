...
const FederalResumeTemplate = ({ userName = "", cvData }) => {
  ...
  if (cvData.contact?.linkedin) contactItems.push(<span key="linkedin"><strong>LinkedIn:</strong> {cvData.contact.linkedin}</span>);
        ...
        <div className="flex flex-wrap justify-center items-center gap-2 text-sm text-gray-800">
          {contactItems.map((item, index) => (
            <React.Fragment key={index}>
              {item}
              {index < contactItems.length - 1 && <span className="text-gray-400 font-normal px-1">|</span>}
            </React.Fragment>
          ))}
        </div>
      </header>
                      ...
                      {section.items.map((item, itemIndex) => {
                        ...
                        return (
                          <div key={itemIndex} className="text-[13px] break-inside-avoid">
                            ...
                            {item.description && <div className="text-gray-700 mt-0.5 whitespace-pre-wrap break-words">{item.description}</div>}
                          </div>
                        );
                      })}
                    </div>
