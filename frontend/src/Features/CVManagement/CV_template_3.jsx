...
const ExecutiveResumeTemplate = ({ userName = "", cvData, highlights = {} }) => {
  ...
  const SectionHeader = ({ title }) => (
    <div className="border-y-2 border-gray-300 py-1.5 mb-2.5 mt-5">
      <h2 className="text-[13px] font-bold uppercase text-gray-800 tracking-wide">
        {title}
      </h2>
    </div>
  );
  ...
  return (
    ...
    >
      <header className="text-center mb-5">
        <h1 className="text-[26px] md:text-[30px] lg:text-[34px] font-bold text-gray-800 mb-1.5">
          {userName}
        </h1>
                    ...
                    return (
                      <div key={index} className="break-inside-avoid" style={getHighlightStyle(`experience_${index}_institutionName`, `experience_${index}_position`, `experience_${index}_summary`)}>
                        <div className="flex flex-col sm:flex-row justify-between items-baseline mb-0.5">
                          ...
                          {dur && (
                            <div className="text-xs md:text-sm text-gray-800 font-bold sm:text-right mt-1 sm:mt-0">
                              {dur}
                            </div>
                          )}
                        </div>
                        {exp.summary && (
                          <div className="text-xs md:text-sm text-gray-800 leading-relaxed whitespace-pre-wrap break-words mt-1.5">
                            {exp.summary}
                          </div>
                        )}
                      </div>
                    ...
                    return (
                      <div key={itemIndex} className="break-inside-avoid">
                        <div className="flex flex-col sm:flex-row justify-between items-baseline mb-0.5">
                          ...
                          {dur && (
                            <div className="text-xs md:text-sm text-gray-800 font-medium sm:text-right">
                              {dur}
                            </div>
                          )}
                        </div>
                        {item.description && (
                          <div className="text-xs md:text-sm text-gray-800 leading-relaxed whitespace-pre-wrap break-words">
                            {item.description}
                          </div>
                        )}
                      </div>
