        ...
        const sectionBlocks = {
          summary: cvData.summary ? (
            <section key="summary" className="break-inside-avoid mb-6">
              <h3 className="text-sm md:text-base font-bold uppercase tracking-wider text-gray-900 mb-1">Objective</h3>
              <hr className="border-t-2 border-gray-300 mt-1 mb-3" />
              <p className="text-[13.5px] md:text-sm text-gray-800 leading-relaxed whitespace-pre-wrap break-words">{cvData.summary}</p>
            </section>
          ) : null,
          education: cvData.education && cvData.education.length > 0 ? (
            <section key="education" className="break-inside-avoid mb-6">
              <h3 className="text-sm md:text-base font-bold uppercase tracking-wider text-gray-900 mb-1">Education</h3>
              <hr className="border-t-2 border-gray-300 mt-1 mb-3" />
                  ...
                  return (
                    <div key={index} className="break-inside-avoid">
                      <p className="text-[13.5px] md:text-sm text-gray-900">
                        <span className="font-semibold">{edu.certification}</span>
                        {edu.institutionName && ` | ${edu.institutionName}`}
                        {dur && <span className="text-gray-500">{` | ${dur}`}</span>}
                      </p>
                      {edu.summary && <p className="text-[13.5px] md:text-sm text-gray-700 mt-1 leading-relaxed whitespace-pre-wrap break-words">{edu.summary}</p>}
                    </div>
                  );
                })}
              </div>
          ...
          experience: cvData.experience && cvData.experience.length > 0 ? (
            <section key="experience" className="break-inside-avoid mb-6">
              <h3 className="text-sm md:text-base font-bold uppercase tracking-wider text-gray-900 mb-1">Experience</h3>
              <hr className="border-t-2 border-gray-300 mt-1 mb-3" />
              <div className="space-y-4 block">
                {cvData.experience.map((exp, index) => {
                  ...
                  return (
                    <div key={index} className="break-inside-avoid">
                      <p className="text-[13.5px] md:text-sm text-gray-900">
                        <span className="font-semibold">{exp.institutionName}</span>
                        {exp.position && ` | ${exp.position}`}
                        {dur && <span className="text-gray-500">{` | ${dur}`}</span>}
                      </p>
                      {exp.summary && <p className="text-[13.5px] md:text-sm text-gray-700 mt-1 leading-relaxed whitespace-pre-wrap break-words">{exp.summary}</p>}
                    </div>
                  );
                })}
              </div>
          ...
          technicalSkills: cvData.technicalSkills?.length > 0 ? (
            <section key="technicalSkills" className="break-inside-avoid mb-6">
              <h3 className="text-sm md:text-base font-bold uppercase tracking-wider text-gray-900 mb-1">Technical Skills</h3>
              <hr className="border-t-2 border-gray-300 mt-1 mb-3" />
              <p className="text-[13.5px] md:text-sm text-gray-800 leading-relaxed">{cvData.technicalSkills.join(', ')}</p>
            </section>
          ) : null,
          softSkills: cvData.softSkills?.length > 0 ? (
            <section key="softSkills" className="break-inside-avoid mb-6">
              <h3 className="text-sm md:text-base font-bold uppercase tracking-wider text-gray-900 mb-1">Soft Skills</h3>
              <hr className="border-t-2 border-gray-300 mt-1 mb-3" />
              <p className="text-[13.5px] md:text-sm text-gray-800 leading-relaxed">{cvData.softSkills.join(', ')}</p>
            </section>
          ) : null,
          language: cvData.language?.length > 0 ? (
            <section key="language" className="break-inside-avoid mb-6">
              <h3 className="text-sm md:text-base font-bold uppercase tracking-wider text-gray-900 mb-1">Languages</h3>
              <hr className="border-t-2 border-gray-300 mt-1 mb-3" />
              <p className="text-[13.5px] md:text-sm text-gray-800 leading-relaxed">{cvData.language.join(', ')}</p>
            </section>
          ) : null,
        };
          ...
          if (key === 'customSections' && cvData.customSections?.length > 0) {
            return cvData.customSections.map((section, sectionIndex) => (
              <section key={`custom-${sectionIndex}`} className="break-inside-avoid mb-6">
                <h3 className="text-sm md:text-base font-bold uppercase tracking-wider text-gray-900 mb-1">{section.title}</h3>
                <hr className="border-t-2 border-gray-300 mt-1 mb-3" />
                <div className="space-y-4 block">
                  {section.items.map((item, itemIndex) => {
                    ...
                    return (
                      <div key={itemIndex} className="break-inside-avoid">
                        <p className="text-[13.5px] md:text-sm text-gray-900">
                          {item.link ? (
                            <a href={item.link} target="_blank" rel="noopener noreferrer" className="font-semibold text-blue-600 hover:underline">{item.name}</a>
                          ) : (
                            <span className="font-semibold">{item.name}</span>
                          )}
                          {dur && <span className="text-gray-500">{` | ${dur}`}</span>}
                        </p>
                        {item.description && <p className="text-[13.5px] md:text-sm text-gray-700 mt-1 leading-relaxed whitespace-pre-wrap break-words">{item.description}</p>}
                      </div>
                    );
                  })}
                </div>
