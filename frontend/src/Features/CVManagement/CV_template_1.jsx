...
const ResumeTemplate = ({ userName, cvData }) => {
  ...
  const sectionBlocks = {
    summary: cvData.summary ? (
      <section key="summary" className="mb-8 border-b-[1.5px] border-gray-400 pb-8">
        ...
        <p className="text-gray-700 text-sm leading-relaxed text-justify">{cvData.summary}</p>
      </section>
    ) : null,
      ...
      <section key="education" className="mb-8 border-b-[1.5px] border-gray-400 pb-8">
        ...
        <div className="flex flex-col gap-5">
          {cvData.education.map((edu, index) => (
            <div key={index}>
              <p className="text-sm text-gray-500 mb-1">{edu.institutionName} | {edu.duration}</p>
              <h4 className="text-base font-bold text-gray-900 mb-1">{edu.certification}</h4>
              {edu.summary && <p className="text-gray-700 text-sm leading-relaxed text-justify">{edu.summary}</p>}
            </div>
          ))}
        </div>
      </section>
      ...
      <section key="experience" className="mb-8 border-b-[1.5px] border-gray-400 pb-8">
        ...
        <div className="flex flex-col gap-5">
          {cvData.experience.map((exp, index) => (
            <div key={index}>
              <p className="text-sm text-gray-500 mb-1">{exp.institutionName} | {exp.duration}</p>
              <h4 className="text-base font-bold text-gray-900 mb-1">{exp.position}</h4>
              {exp.summary && <p className="text-gray-700 text-sm leading-relaxed text-justify">{exp.summary}</p>}
            </div>
          ))}
        </div>
      </section>
    ...
    skills: allSkills.length > 0 ? (
      <section key="skills" className="mb-4">
        <h3 className="text-lg font-bold uppercase tracking-wider text-gray-900 mb-4">Skills</h3>
        <ul className="grid grid-cols-1 md:grid-cols-3 gap-y-2 gap-x-4 text-sm text-gray-700 list-disc list-inside">
          {allSkills.map((skill, index) => <li key={index} className="marker:text-gray-400">{skill}</li>)}
        </ul>
      </section>
    ) : null,

    technicalSkills: allSkills.length > 0 ? (
  ...
  // Maps combined skills into single block if split isn't heavily ordered
            ...
            <section key={`custom-${sectionIndex}`} className="mb-8 border-b-[1.5px] border-gray-400 pb-8">
              ...
              <div className="flex flex-col gap-5">
                {section.items.map((item, itemIndex) => (
                  <div key={itemIndex}>
                    {item.duration && <p className="text-sm text-gray-500 mb-1">{item.duration}</p>}
                    <h4 className="text-base font-bold text-gray-900 mb-1">
                      {item.name}
                      {item.link && <a href={item.link} target="_blank" rel="noopener noreferrer" className="text-blue-500 text-sm ml-2 font-normal hover:underline">[Link]</a>}
                    </h4>
                    {item.description && <p className="text-gray-700 text-sm leading-relaxed text-justify">{item.description}</p>}
                  </div>
                ))}
              </div>
            </section>
