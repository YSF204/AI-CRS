...
export default function useCVForm(showToast, autoSaveFunction = null, user = null) {
  ...
  const addCustomSection = () => {
    setForm((f) => ({
      ...f,
      customSections: [...f.customSections, newCustomSection()],
    }));
    triggerAutoSave();
  };
  const removeCustomSection = (si) => {
    setForm((f) => ({
      ...f,
      customSections: f.customSections.filter((_, i) => i !== si),
    }));
    triggerAutoSave();
  };
  const updateCustomSectionTitle = (si, val) => {
    setForm((f) => {
      const a = [...f.customSections];
      a[si] = { ...a[si], title: val };
      return { ...f, customSections: a };
    });
    triggerAutoSave();
  };
  const addCustomItem = (si) => {
    setForm((f) => {
      const a = [...f.customSections];
      a[si] = { ...a[si], items: [...a[si].items, newCustomItem()] };
      return { ...f, customSections: a };
    });
    triggerAutoSave();
  };
  const removeCustomItem = (si, ii) => {
    setForm((f) => {
      const a = [...f.customSections];
      a[si] = { ...a[si], items: a[si].items.filter((_, i) => i !== ii) };
      return { ...f, customSections: a };
    });
    triggerAutoSave();
  };
    ...
    async (field, context = {}) => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          console.error(
            `Suggestion API error for ${field}:`,
            response.status,
            errorData,
          );
  ...
  const handleSuggestionSelect = useCallback(
    (field, value) => {
      if (field === "job-title" || field === "jobTitle") {
        ...
        // Handle language objects with {name, level} format
        const langArray = form.language || [];
        const langNames = langArray.map((l) =>
          typeof l === "string" ? l : l.name,
        );
        if (!langNames.includes(value)) {
          const newLang =
            typeof value === "string" ? { name: value, level: "A1" } : value;
  ...
  const getFilteredFormData = useCallback(
    (activeSections) => {
      ...
      if (!activeSections.includes("contact")) {
        f.contact = { phone: "", email: "", github: "", linkedin: "" };
      }
      if (!activeSections.includes("address")) {
        f.address = { city: "", street: "" };
      }
      ...
      if (!activeSections.includes("technicalSkills")) {
        f.technicalSkills = [];
      }
      if (!activeSections.includes("softSkills")) {
        f.softSkills = [];
      }
      if (!activeSections.includes("language")) {
        f.language = [];
      }
      if (!activeSections.includes("customSections")) {
        f.customSections = [];
      }
  ...
  const handleImageUpload = (e) => {
    ...
    if (!file) return;
    if (file.size > 512000) {
      showToast("error", "Image must be under 500KB.");
      return;
    }
