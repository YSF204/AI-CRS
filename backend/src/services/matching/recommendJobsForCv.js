...
const buildLocalJobRecommendations = (cv, jobs, normalizedProfile) => {
    const cvTech = new Set(
        (normalizedProfile?.technicalSkills || cv.technicalSkills || []).map((skill) => skill.toLowerCase()),
    );
    const cvSoft = new Set(
        (normalizedProfile?.softSkills || cv.softSkills || []).map((skill) => skill.toLowerCase()),
    );
    const cvExperience = normalizedProfile?.yearsOfExperience ||
        (cv.experience || []).reduce((sum, exp) => sum + Number(exp.duration || 0), 0);

    return jobs
        .map((job) => {
            const jobTech = job.technicalSkills || [];
            const jobSoft = job.softSkills || [];
            const techMatched = jobTech.filter((skill) => cvTech.has(skill.toLowerCase()));
            const softMatched = jobSoft.filter((skill) => cvSoft.has(skill.toLowerCase()));
            const experienceScore = job.yearsOfExperience
                ? Math.min(100, Math.round((cvExperience / job.yearsOfExperience) * 100))
                : 100;
            ...
            return {
                ...
                matchScore: Math.min(
                    100,
                    Math.round(techMatched.length * 3 + softMatched.length * 1.5 + experienceScore * 0.2),
                ),
                skillsMatched: [...new Set([...techMatched, ...softMatched])],
                skillsMissing: jobTech.filter((skill) => !cvTech.has(skill.toLowerCase())),
...
const normalizeAiMatches = (parsedMatches, jobs) => {
    ...
    const jobsByTitle = new Map(jobs.map((job) => [job.position?.toLowerCase().trim(), job]));
...
export const recommendJobsForCv = async ({ cvId, userId }) => {
    ...
    const candidateIds = new Set(localRanked.slice(0, 35).map((job) => job.jobId));
    ...
    let rankedMatches;
    try {
        const aiMatch = await withTimeout(matchCVToJobs(formatCvText(cv), jobsText), 25000);
        const clean = aiMatch.replace(/```json|```/g, "").trim();
